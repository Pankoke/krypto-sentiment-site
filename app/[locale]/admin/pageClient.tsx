        ...
        <section className="mx-auto max-w-3xl px-4 py-12">
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <p className="mt-2 text-sm text-slate-600">
              In dieser Umgebung ist der Admin-Bereich nur mit einem gültigen Login verfügbar.
            </p>
          </div>
        </section>
      </main>
      ...
      <section className="mx-auto max-w-6xl px-4 py-10 md:py-12">
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="max-w-2xl text-sm text-slate-600">
                Übersicht über Daily-Run, News-Snapshots, Logs und historische Daten. Nutze die Aktionen mit Bedacht –
                sie triggern echte Backend-Prozesse.
              </p>
            </div>
            <Button
              onClick={async () => {
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <div className="space-y-6">
            <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              ...
              <div className="mt-3 space-y-2 text-xs text-slate-700">
                ...
                {historyError && (
                  <p className="text-rose-700">Fehler beim Laden der Snapshot-Historie. Bitte später erneut versuchen.</p>
                )}
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              ...
              <div className="mt-4 space-y-3">
                <div className="space-y-1">
                  ...
                  >
                    {actionState.dailyRun.loading ? "Läuft..." : "Daily-Run anstoßen"}
                  </Button>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm lg:col-span-1">
            ...
            <div className="mt-4 space-y-4">
              ...
              <div className="rounded-lg border border-slate-100 bg-slate-50 p-3">
                <h3 className="text-xs font-semibold text-slate-800">Asset-Historie (30 Tage)</h3>
                <div className="mt-2 flex items-center gap-3 text-xs text-slate-700">
                  <label className="font-semibold" htmlFor="asset-select">
                    Asset:
                  </label>
                  <select
                    id="asset-select"
                ...
                <div className="mt-2 h-40 overflow-auto">
                  {isAssetLoading && <p className="text-sm text-slate-500">Lade Asset-Historie...</p>}
                  {assetError && <p className="text-sm text-rose-700">Fehler: {assetError}</p>}
                  {!isAssetLoading && !assetError && (
                    <ul className="space-y-1 text-xs text-slate-700">
                      {assetHistory.length === 0 && <li className="text-slate-500">Keine Asset-Historie verfügbar.</li>}
                      {assetHistory.map((point) => (
                        <li key={`${point.date}-${point.score}`} className="flex items-center justify-between">
                          <span>{point.date}</span>
                          <span className="font-semibold text-slate-900">{point.score.toFixed(3)}</span>
                        </li>
                      ))}
                    </ul>
                  )}
        ...
        <section className="mt-8 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="mb-4 flex items-center justify-between gap-2">
            <h2 className="text-sm font-semibold text-slate-900">Sentiment-Snapshots</h2>
            <p className="text-xs text-slate-500">Aktuelle Items aus der Sentiment-API.</p>
          </div>
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {sentimentResponse?.items?.length ? (
              sentimentResponse.items.map((item) => <SentimentCard key={item.symbol} item={item} />)
            ) : (
              <p className="text-sm text-slate-500">Keine Sentiment-Daten vorhanden.</p>
            )}
          </div>
        </section>
      </section>
