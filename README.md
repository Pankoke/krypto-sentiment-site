...
- Die Liste der verfügbaren Assets wird zentral in `data/assets.json` gepflegt. Nur Einträge mit `enabled: true` dürfen auf Dashboard, Archiv und APIs erscheinen; derzeit sind das `BTC`, `ETH`, `SOL` und `XRP`.
- Neue Assets kommen hinzu, indem du einfach einen neuen Eintrag im JSON ergänzt und `enabled: true` setzt. Nach dem Deployment verwenden Aggregator, APIs und UI automatisch die aktualisierte Whitelist, ohne weiteren Code-Refactor.
- Alle nicht erlaubten Pfade (z. B. `/asset/avax`) landen mit `redirect=invalid-asset` wieder auf der Lokalisierungshomepage. So erzeugen Sitemaps, Navigation und Deep-Links nur noch die vier aktiven Märkte.
